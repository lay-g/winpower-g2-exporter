...
**设计原则**：遵循 Prometheus 指标规范，提供简洁、标准化的指标管理，与已实现的 winpower、energy、storage、logging 模块紧密集成，支持完整的可观测性需求。
...
4. **集成测试**: 与已实现模块正确集成，数据流转正常
5. **错误处理**: 优雅处理各种异常情况，不影响其他模块
